...
class ParsePDFDocTextImages():
    def __init__(self, s3_c, config, s3_pdf_path,s3_output_folder):
        """
    ...
    def get_block_images(self, chunk_number,block, block_details, accumulated_images, page_details):
    ...
    def process_pdf_page(self, page_indx, page):
        ...
        if "blocks" in text_data.keys():
            for block in text_data["blocks"]:
                ...
                if "image" in block:
                    accumulated_images, page_details, img_indx = self.get_block_images(chunk_number, block, block_details, accumulated_images, page_details)
