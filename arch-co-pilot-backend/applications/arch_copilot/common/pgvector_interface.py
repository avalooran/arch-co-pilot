...
class PGVectorInterface():
    ...
    @timeit 
    def delete_table_records(self,table_name, delete_tables,search_column, search_value):
        ...
        if search_values:
            for search_value in search_values:
                for table_name in delete_tables:
                    response = self.delete_table_records(table_name, delete_tables,uk_column, search_value[f'{uk_column}'])
    ...
    def get_cosine_topn_similar_records(self, top_n, table_name, similarity_vector, min_threshold=0.6):
        sql_stmnt = f"""SELECT 1 - (multimodal_embedding <=> '{similarity_vector}') as cosine_similarity, 
                        embbeding_id, embbeding_type, doc_details, document_source, document_id, document_name, 
                        s3_document_path, s3_image_path, image_description, image_base64
                        FROM {table_name} where embbeding_type = 'image' 
                        and (1 - (multimodal_embedding <=> '{similarity_vector}')) > {min_threshold} 
                        ORDER BY 1 - (multimodal_embedding <=> '{similarity_vector}') DESC 
                        LIMIT { top_n}; """
        response = self.execute_statement(sql_stmnt) 
        return self.formatOutputJsonRecords(response)

    def get_l2_topn_similar_records(self, top_n, table_name, similarity_vector, max_threshold=0.8):
        sql_stmnt = f"""SELECT multimodal_embedding <-> '{similarity_vector}' as l2_similarity, 
                        embbeding_id, embbeding_type, document_category, document_source, document_id, document_name, 
                        s3_document_path, s3_image_path, image_description,image_base64
                        FROM {table_name} where embbeding_type = 'image' 
                        and (multimodal_embedding <-> '{similarity_vector}') < {max_threshold}
                        ORDER BY multimodal_embedding <-> '{similarity_vector}' ASC 
                        LIMIT { top_n}; """
        response = self.execute_statement(sql_stmnt) 
        return self.formatOutputJsonRecords(response)
